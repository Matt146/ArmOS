...
#define BITMAP_MEMORY_SIZE 0x200000 // The size of memory each bit in a bitmap byte entry points to
...
void pmm_set_bitmap_block_used(uint64_t block); // sets the block in the memory map to used
void pmm_alloc(uint64_t blocks);                // allocates "blocks" blocks
