...
void vmm_map_page(uint64_t vaddr, uint64_t paddr, uint16_t flags) {
    ...
    serial_puts(" to paddr ");
    serial_puts(unsigned_long_to_str(paddr));
    serial_puts(" with flags ");
    serial_puts(unsigned_long_to_str(flags));
    */
    ...
    /*
    serial_puts("\n - [-] P4: ");
    serial_puts(unsigned_long_to_str(p3));
    serial_puts("\n - [-] P3: ");
    serial_puts(unsigned_long_to_str(p2));
    serial_puts("\n - [-] P1: ");
...
/*
void vmm_map_page(uint64_t vaddr, uint64_t paddr, uint16_t flags) {
    // Begin with some debug messages
    serial_puts("\n[VMM] Mapping vaddr ");
    serial_puts(unsigned_long_to_str(vaddr));
    serial_puts(" to paddr ");
    serial_puts(unsigned_long_to_str(paddr));
    ...
    // Print out the indices to make sure you didn't fuck up
    serial_puts("\n - [-] Indices");
    serial_puts("\n\t - [-] P4: ");
    serial_puts(unsigned_long_to_str(p4_idx));
    serial_puts("\n\t - [-] P3: ");
    serial_puts(unsigned_long_to_str(p3_idx));
    serial_puts("\n\t - [-] P2: ");
    serial_puts(unsigned_long_to_str(p2_idx));
    serial_puts("\n\t - [-] P1: ");
    serial_puts(unsigned_long_to_str(p1_idx));
    ...
    if (p4[p4_idx] == NULL) {
        p3 = (uint64_t*)pmm_alloc(1);
        memsetb(p3, 0, 0x1000);
        serial_puts("\n\t - [-] P3 does not exist... creating one at ");
        serial_puts(unsigned_long_to_str(p3));
        p4[p4_idx] = (uint64_t)p3 | 0x3;  // Or with 0x3 to mark page as present and readable/writable
        serial_puts("\n\t - [-] P4 entry set to ");
        serial_puts(unsigned_long_to_str((uint64_t)p3 | 0x3));
    } else {
        // Else, a P4 entry does exist, so we just set our p3 pointer to the existing
        // lower-level page table
        serial_puts("\n\t - [-] P3 exists.");
        serial_puts("\n\t - [-] P4 entry set to ");
        serial_puts(unsigned_long_to_str(p4[p4_idx]));
        p3 = (uint64_t*)(p4[p4_idx] & 0x3fffffffffffff);
    }
    serial_puts("\n\t - [-] p4[p4_idx] value ");
    serial_puts(unsigned_long_to_str(p4[p4_idx]));
    ...
    if (p3[p3_idx] == NULL) {
        p2 = (uint64_t*)pmm_alloc(1);
        memsetb(p2, 0, 0x1000);
        serial_puts("\n\t - [-] P2 does not exist... creating one at ");
        serial_puts(unsigned_long_to_str(p2));
        p3[p3_idx] = (uint64_t)p2 | 0x3;
        serial_puts("\n\t - [-] P3 entry set to ");
        serial_puts(unsigned_long_to_str((uint64_t)p2 | 0x3));
    } else {
        // Else, a P3 entry does exist, so just set our p2 pointer to the existing
        // lower-level page table
        serial_puts("\n\t - [-] P2 exists.");
        serial_puts("\n\t - [-] P3 entry set to ");
        serial_puts(unsigned_long_to_str(p3[p3_idx]));
        p2 = (uint64_t*)(p3[p3_idx] & 0x3fffffffffffff);
    }
    serial_puts("\n\t - [-] p3[p3_idx] value ");
    serial_puts(unsigned_long_to_str(p3[p3_idx]));
    ...
    serial_puts(unsigned_long_to_str(p2));
    serial_puts("\n\t - [-] p2[p2_idx] value ");
    serial_puts(unsigned_long_to_str(p2[p2_idx]));
    if (p2[p2_idx] == NULL) {
        p1 = (uint64_t*)pmm_alloc(1);
        memsetb(p1, 0, 0x1000);
        serial_puts("\n\t - [-] P1 does not exist... creating one at ");
        serial_puts(unsigned_long_to_str(p1));
        p2[p2_idx] = (uint64_t)p1 | 0x3;
        serial_puts("\n\t - [-] P2 entry set to ");
        serial_puts(unsigned_long_to_str((uint64_t)p1 | 0x3));
    } else {
        // Else, a P2 entry does exist, so just set our p1 pointer to the existing
        // lower-level page table
        serial_puts("\n\t - [-] P1 exists.");
        serial_puts("\n\t - [-] P2 entry set to ");
        serial_puts(unsigned_long_to_str(p2[p2_idx]));
        p1 = (uint64_t*)(p2[p2_idx] & 0x3fffffffffffff);
    }
    serial_puts("\n\t - [-] p2[p2_idx] value ");
    serial_puts(unsigned_long_to_str(p2[p2_idx]));
    ...
    serial_puts("\n - [-] Mapping P1 --> PADDR");
    serial_puts("\n\t - [-] P1 entry set to ");
    serial_puts(unsigned_long_to_str(paddr | flags));
    p1[p1_idx] = paddr | flags;
}
*/